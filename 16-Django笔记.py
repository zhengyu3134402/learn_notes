...
#                     # from django.utils.decorators import method_decorator
    ...
    # 12 同源策略
        ...
        # 1 概念
            ...
            # 同源策略是浏览器的一个安全功能，不同源的客户端脚本在没有明确授权的情况下，
                # 不能读写对方资源。所以xyz.com下的js脚本采用ajax读取abc.com里面的文件数据
                # 是会被拒绝的。
        ...
        # 2 不同源数据获取
            # 1 通过script标签的跨域特性来绕过同源策略拿到想要的数据
                # 1 定义一个变量
                    # 后端
                        # def abc(request):
                ...
                # 3 函数传参
                    # 后端
                        # def get(self, request):
                            # a = {"a": 123, "b": [1, "s", 'g']}
                            # return HttpResponse('sss(%r)' % (json.dumps(a)))
                    # 前端
            ...
            # 通过jsonp 来获取数据
                # 1 概念
                    ...
                    # 但是我们更多时候是希望通过事件触发数据的获取，而不是像上面一样页面一
                    # 刷新就执行了，这样很不灵活。
                    # 其实这很好解决，我们可以通过javascript动态的创建script标签来实现。
                # 2 使用
                    # 1 点击按钮，会在页面上插入一个script标签，然后从后端获取数据
                        ...
                        #     </script>
                        #     </body>
                    # 2 客户端定义的回调函数的函数名传给服务端

                        # 为了实现更加灵活的调用，我们可以把客户端定义的回调函数的函数名传给服务端，
                        # 服务端则会返回以该回调函数名，将获取的json数据传入这个函数完成回调。
                        # 后端
                            # def abc(request):
                    ...
                    # 3 jQuery中getJSON方法
                        ...
                        # 要注意的是在url的后面必须要有一个callback参数，这样getJSON方法才会知
                        # 道是用JSONP方式去访问服务，callback后面的那个？是jQuery内部自动生成的一个回调函数名。

                    # 4 但是如果我们想自己指定回调函数名，使用$.ajax
    ...
    # 16 Session
        ...
        # 2 session操作
            # 1 增
                ...
                # 2 设置session的键值，存在则不设置
                    # request.session.setdefault('k1',123)
                # 3设置会话Session和Cookie的超时时间
            ...
            # 2 删
                ...
                # 2 将所有Session失效日期小于当前日期的数据删除
                    # request.session.clear_expired()
                # 3 删除当前会话的所有Session数据
                    # request.session.delete()
                # 4 删除当前的会话数据并删除会话的Cookie。
    ...
    # 19 django的cbv
        ...
        # 1 CBV中的dispatch方法
            ...
            #     def post(self, request)
            #         ...
        # 2 cbv单独添加装饰器到get方法上（单独加在post方法上也是一样）
            #     知识点
        ...
        # 4 cbv将装饰器加载视图类之上
            ...
            1
    # 20 django的HttpResponse,render,redirect,HttpRequest(request)
        1
        # 1 HttpResponse
            #     知识点
